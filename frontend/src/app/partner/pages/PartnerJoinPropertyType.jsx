  ...
  useEffect(() => {
    const buildFourOptions = async () => {
      try {
        ...
        const basePath = location.pathname.startsWith('/list-property') ? '/list-property' : '/hotel';

        // Construct the 4 options with IDs if available
        const newOptions = [
          {
            ...FOUR_OPTIONS[0],
            route: `${basePath}/join-pg`, // specific route for PG
            categoryId: pgCat?._id
          },
          {
            ...FOUR_OPTIONS[1],
            route: rentCat ? `${basePath}/wizard/${rentCat._id}` : null,
            categoryId: rentCat?._id
          },
          {
            ...FOUR_OPTIONS[2],
            route: buyCat ? `${basePath}/wizard/${buyCat._id}` : null,
            categoryId: buyCat?._id
          },
          {
            ...FOUR_OPTIONS[3],
            route: plotCat ? `${basePath}/wizard/${plotCat._id}` : null,
