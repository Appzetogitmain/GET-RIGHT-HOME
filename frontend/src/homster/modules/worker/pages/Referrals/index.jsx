  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="bg-white px-4 py-4 sticky top-0 z-50 shadow-sm flex items-center gap-4">
        ...
        <h1 className="text-xl font-bold text-gray-800">My Referrals</h1>
      </div>
      ...
      {loading ? (
        ...
        <div className="p-4 space-y-6">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg">
            <h2 className="text-lg font-medium mb-1 opacity-90">Your Referral Code</h2>
            <div className="flex items-center justify-between bg-white/20 rounded-xl p-3 mt-4 border border-white/30 backdrop-blur-sm">
              <span className="text-2xl font-bold tracking-widest">{data.referralCode || 'N/A'}</span>
              <button 
                onClick={handleCopy}
            ...
            <button 
              onClick={handleShare}
              className="w-full mt-4 flex items-center justify-center gap-2 py-3 bg-white text-blue-600 rounded-xl font-bold hover:bg-gray-50 transition-colors shadow-sm"
            >
              <FiShare2 className="w-5 h-5" /> Share with Friends
            </button>
          </div>
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex items-center gap-4">
            ...
            <div>
              <p className="text-gray-500 text-sm font-medium">Total Referred Workers</p>
              <p className="text-2xl font-bold text-gray-800">{data.totalReferrals}</p>
            </div>
          </div>

          {/* Referrals List */}
          <div>
            <h3 className="font-bold text-gray-800 mb-4 px-1">Referred Friends</h3>
            {data.referrals.length > 0 ? (
                ...
                {data.referrals.map((referral, index) => (
                  <div key={index} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-bold text-gray-800 truncate">{referral.name}</h4>
              ...
              <div className="bg-white rounded-xl p-8 text-center border border-gray-100 shadow-sm">
                <div className="w-16 h-16 bg-blue-50 text-blue-300 rounded-full flex items-center justify-center mx-auto mb-3">
                  <FiUsers className="w-8 h-8" />
                </div>
                <p className="text-gray-500 font-medium">You haven't referred anyone yet.</p>
                <p className="text-sm text-gray-400 mt-1">Share your code to invite friends.</p>
              </div>
            )}
          </div>
        </div>
